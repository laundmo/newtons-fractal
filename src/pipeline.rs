...
impl Plugin for ShaderPipelinePlugin {
    fn build(&self, app: &mut App) {
        ...
        render_app
            ...
            .insert_resource(ConfigMeta { buffer })
            .add_system(prepare_time.in_set(RenderSet::Prepare))
...
fn prepare_time(
    config: Res<ShaderParams>,
    config_meta: ResMut<ConfigMeta>,
    ...
    render_queue.write_buffer(
        ...
        0,
        &[
            bevy::core::cast_slice(&[config.extents]),
            bevy::core::cast_slice(&[config.size]),
        ]
        .concat(),
    );
}
